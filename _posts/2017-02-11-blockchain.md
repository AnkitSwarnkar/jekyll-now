...
A blockchain is simply a distributed ledger system. Block chain is now being recognized as a new era for every feilds. This disruptive technology has created lot of buzz in multiple domains like financial, IOT, academic, assest management etc. Many big tech companies like IBM, Disney, Intel, Samsung have started leveraging this amazing technology. In this post I will try to summarize the most important part of blockchain which is the consensus protocol also known as the heart of blockchain. 

So what is a consesus protocol?
The term ‘consensus’ refers to a decision-making process which is done in a collabrative way. Generally, for centralized architecture there is a centralized machine/node which takes all the decision, however for block chain processing is quite different. Being a distributive architecture, there is a need of a set of rule by which various different computers/nodes connected with each other come to an agreement. More technically, consensus algorithm is required by the blockchain to ensure that every node has a copy of a recognized version of the total ledger.[1] Consensus algorithm allows secure updating of state according to some rules or conditions.  

Since the inception of PBFT consensus algorithm, there are plethora of consensus algorithms created to solve the Byzantine problems. Some of the known protocols are proof of Work, proof of stake, proof of concepts and the list continue. In this post I will try to introduce to some of the amazing new consensus protocols that are becoming talk of the industry. 

#### 1. Tangaroa

#### Introduction
Tangaroa consesus protocol or BFT Raft is a Byzantine server tolerance enhancement to the Raft algorithms i.e. it try to mitigate the risk of byzantine nodes in previous RAFT[https://raft.github.io/] implementation. The Tangaroa consensus algorithm just like RAFT algorithm divides the consensus algorithm into two problems of log replication and leader selection. A BFT Raft cluster that tolerates f Byzantine failures must contain at least n ≥ 3f + 1 nodes, where n − f nodes form a quorum. At any given time the node can be in only three states of leader,follower, or candidate. 
